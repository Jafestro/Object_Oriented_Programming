...
public class CSVFromURL {
    public static void main(String[] args) {
        ...
        try {
            ...
            double sum = 0;
            int numCount = 0;
            String startFrom = "01.01.2023";
            String till = "02.01.2023";
            ...
            while((line = bufferedReader.readLine()) != null){
                ...
                if(Arrays.asList(list[0].split(" ")).contains(startFrom)) { // ["01.01.2023 00:0...", "5,0", "21,4", "25,0", "25,5", +16 more]
                    String rawNum = list[1].replace(',', '.'); // 5,0 -> 5.0
                    double num = Double.parseDouble(rawNum);
                    sum += num;
                    numCount++;
                } else if (Arrays.asList(list[0].split(" ")).contains(till)) {
                    break;
                }
            }
            double roundedAverage = Math.round((sum/numCount) * 100.0) / 100.0;
            System.out.println("Average temperature for " + startFrom + " is " + roundedAverage + " degrees in Celsius");
